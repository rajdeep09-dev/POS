      ...
      <motion.div variants={container} initial="hidden" animate="show" className="grid gap-6 md:grid-cols-3">
        {stats.map((stat, i) => (
          <motion.div key={i} variants={item}>
            <Card className="border-zinc-200 shadow-xl rounded-2xl overflow-hidden group">
              <CardContent className="p-6 flex justify-between items-start">
                ...
                <div className="p-3 bg-zinc-50 rounded-xl border border-zinc-100"><stat.icon className={`h-5 w-5 ${stat.color}`} /></div>
              </CardContent>
            </Card>
          </motion.div>
        ))}
